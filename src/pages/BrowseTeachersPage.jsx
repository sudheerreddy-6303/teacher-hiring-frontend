...
export default function BrowseTeachersPage({ setPage }) {
  ...
  const activeFilterCount = ["subject","city","experience","mode"].filter(k => filter[k] !== "All").length
    + (filter.search ? 1 : 0);
                    ...
                    {f.options.map(o => (
                      <label key={o}
                        style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 8px", borderRadius:7, cursor:"pointer", fontSize:13.5,
                          color: filter[f.key]===o ? "#1A56DB" : "#374151", fontWeight: filter[f.key]===o ? 700 : 500,
                          background: filter[f.key]===o ? "#EBF5FF" : "transparent" }}>
                        <input type="radio" name={`teacher-${f.key}`} checked={filter[f.key]===o}
